...
trait Model {
  ...
  protected def getValueOrRaiseError[F[_]: Sync, A](form: UrlForm, value: String): F[A] =
    form.getFirst(value).fold(Sync[F].raiseError[A](MalformedMessageBodyFailure(s"forgot \$value")))(Sync[F].pure)
}
